...
const Dashboard = (CUId) => {
  ...
  const getExamNames = async () => {
    const { data } = await axios.get(`http://localhost:3000/exam/${CUId.CUId}`);
  ...
  const deleteExam = (id) => {
    ...
    axios.delete(`http://localhost:3000/exam/${id}`, { headers }).then((response) => {
  ...
  const handleName = (e) => {
    ...
    if (examName == "") {
      ...
      console.log(newExam)
      axios.post("http://localhost:3000/exam/", newExam).then((response) => {
        console.log(response.status);
        console.log(response.data);
      });
      setDummy(dummy + 1)
              ...
              <TableBody>
                {examNameStorage.map((name) => (
                  ...
                  >
                    ...
                    <TableCell align="right"><Link to={`/analyze/${name._id}`}><Button><BarChart style={{ verticalAlign: "middle", padding: "5px" }} />Analyze</Button></Link></TableCell>
