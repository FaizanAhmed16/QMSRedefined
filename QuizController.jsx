...
const QuizController = (CUId) => {
    ...
    const getExams = async () => {
        const { data } = await axios.get('http://localhost:3000/examquestions/' + id.id);
        setQuestions(data);
        userCheck();
    }
    ...
    const securityData = async () => {
        axios.all([
            await axios.get('http://localhost:3000/users/' + CUId.CUId),
            await axios.get('http://localhost:3000/exam/exam/' + id.id)
            ...
            if (data2.data[0].creatorUserId == CUId.CUId) {
                ...
                axios.post("http://localhost:3000/userexams/", dummyData).then((response) => {
    ...
    const userCheck = async () => {
        try {
            const { data } = await axios.get('http://localhost:3000/userexams/' + CUId.CUId);
